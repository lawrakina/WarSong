...
namespace Code.Extension
{
    public class LayerManager : MonoBehaviour
    {
        ...
        [SerializeField] private List<string> _needLayers = new List<string>()
        {
            "Enemy",
            "EnemyAttack",
            "Player",
            "PlayerAttack",
            "EnemyAndPlayerAttack"
        };
        ...
        {
            Dbg.Log($" Checking required layers ");
            foreach (var layer in _needLayers)
            {
                var result = LayerMask.NameToLayer(layer);
                if (result < 0)
                {
                    AddNewLayer(layer);
                    Dbg.Log($" Layer:'{layer}' added!");
                }
                else
                {
                    Dbg.Log($" Layer:'{layer}' has already been added");
                }
            }
            Dbg.Log($" Checking required layers complete.");
        }
